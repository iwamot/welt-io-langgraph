...
- `sample_dangerous_action` — a pretend dangerous action (no side effects, no extra AWS permissions) gated by `HumanInTheLoopMiddleware`: the tool itself carries no interrupt code, and the run pauses before its body starts. Welt renders **Approve** / **Reject** buttons plus a free-text field in the Slack thread, and whichever answer comes first resumes the run — a press decides whether the tool runs, while typed text answers on the tool's behalf without running it. Ask "deploy to prod", then press a button or type something like "not now". See [Welt's Interrupts doc](https://github.com/iwamot/welt/blob/main/docs/interrupts.md) for the round trip.
