...
- **`edit` has no widget**, rewriting an action's arguments being a form the wire has no shape for. An action allowing `edit` alongside others is asked about with the widgets for the rest. One allowing `edit` alone has no widget to ask with, so the whole request — every action in it, however the others are configured — is passed through to Welt's fallback rendering, whose answers the middleware cannot resume from.
